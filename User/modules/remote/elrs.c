...
#define ELRS_SW_DIV 500//利用C语言中`/`的性质匹配开关状态
...
static void ELRS_UARTE_RxCallback()
{
    ...
    if (elrs_data_temp[0] == CRSF_ADDRESS_FLIGHT_CONTROLLER)
    {
        if (elrs_data_temp[2] == CRSF_FRAMETYPE_RC_CHANNELS_PACKED) // 数据帧类型为RC通道数据
        {
            ...
            elrs_data.A = (elrs_data.channels[4] - 800) / ELRS_SW_DIV;
            elrs_data.B = (elrs_data.channels[5] - 800) / ELRS_SW_DIV;
            elrs_data.C = (elrs_data.channels[6] - 800) / ELRS_SW_DIV;
            elrs_data.D = (elrs_data.channels[7] - 800) / ELRS_SW_DIV;
